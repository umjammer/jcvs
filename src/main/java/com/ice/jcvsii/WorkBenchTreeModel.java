...
public class WorkBenchTreeModel extends DefaultTreeModel {
    ...
    public void recursiveSave(UserPrefs prefs, WorkBenchTreeNode rootNode) {
        if (false) System.err.println("SAVE WORKBENCH NODE: " + rootNode.getDefinition().getFullPath());
        ...
        if (!def.isFolder()) {
            ...
            for (WorkBenchTreeNode child : childs) {
    ...
    public void recursiveLoad(UserPrefs prefs, WorkBenchTreeNode rootNode) {
        ...
        for (String name : chNames) {
            ...
            String localRoot = isFolder ? null : prefs.getProperty(propName + ".local", null);
            ...
            if (isFolder) {
                WorkBenchDefinition childDef = new WorkBenchDefinition(name, rootPath, display, desc);

                WorkBenchTreeNode newNode = new WorkBenchTreeNode(childDef);

                rootNode.add(newNode);

                this.recursiveLoad(prefs, newNode);
            } else {
                WorkBenchDefinition childDef = new WorkBenchDefinition(name, rootPath, display, desc, localRoot);
