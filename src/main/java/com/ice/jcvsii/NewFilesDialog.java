...
public class NewFilesDialog extends JDialog implements ActionListener, ListSelectionListener {
    ...
    public NewFilesDialog(Frame parent, boolean modal, String prompt) {
        ...
        this.ignoreStr = Config.getPreferences().getProperty(Config.GLOBAL_USER_IGNORES, null);
    ...
    public void refreshFileList(File dirF, CVSEntry dirEntry) {
        ...
        String ignoreName = prefs.getProperty(Config.GLOBAL_IGNORE_FILENAME, null);
        ...
        for (String fileName : files) {
            ...
            if (fileName.equals(ignoreName)) continue;

            if (fileName.equals(".cvsignore")) continue;

            if (dirIgnore.isFileToBeIgnored(fileName)) continue;

            CVSEntryList eV = this.dirEntry.getEntryList();

            if (this.dirEntry.locateEntry(fileName) != null) continue;

            this.fileV.add(fileName);
        }

        this.fileList.setListData(this.fileV.toArray(new String[0]));
    }

    public String[] getSelectedFiles() {
        if (this.okClicked) {
            return this.fileList.getSelectedValuesList().toArray(new String[0]);
        } else {
            return new String[0];
        }
    }
    ...
    public void valueChanged(ListSelectionEvent evt) {
        this.okButton.setEnabled(this.fileList.getSelectedIndex() != -1);
    }
    ...
    public void establishDialogContents(String prompt) {
        ...
        promptLabel.setFont(prefs.getFont("newFileDialog.prompt.font", new Font("Dialog", Font.BOLD, 14)));

        this.fileList = new JList<>();
        this.fileList.addListSelectionListener(this);
        this.fileList.setFont(prefs.getFont("newFileDialog.text.font", new Font("Dialog", Font.BOLD, 12)));
        ...
        this.ignoreText.addActionListener(evt -> refreshFileList(dirFile, dirEntry));
