...
public class UserPrefs extends Properties {
     ...
     */
    private static Map<Object, Object> propsTable;
    ...
    protected Map<Object, Object> subScribers;
     ...
     */
    static {
        UserPrefs.propsTable = new HashMap<>();
    ...
    public void setPropertyPrefix(String prefix) {
        ...
        if (this.debug) System.err.println("UserPrefs.setPropertyPrefix: prefix set to '" + this.prefix + "'");
    }
    ...
    public String getCurrentDirectory() {
        ...
        if (result == null) result = ((File.separatorChar == ':') ? "" : ".");
        return result;
    }
    ...
    private String normalizeKey(String key) {
        ...
        for (int i = 0, sz = key.length(); i < sz; ++i) {
            ...
            if (Character.isLetterOrDigit(ch)) {
                ...
                int hexDig = (chVal & 15);
                kBuf.append(hexDig < 9 ? (char) ('0' + hexDig) : (char) ('A' + (hexDig - 10)));
    ...
    public String normalizedPropertyName(String name) {
        ...
        else return this.prefixedPropertyName(name);
    }
     ...
     * is not defined.
     *
     * @param normName The name of the property to retrieve.
    ...
    private String locateProperty(Properties props, String normName) {
        ...
        if (this.osSuffix != null && this.userSuffix != null) {
            ...
            if (this.debug) System.err.println("UserPrefs.getOverridableProperty: " + overName + " = '" + value + "'");
        ...
        if (value == null) {
            ...
            if (this.debug) System.err.println("UserPrefs.getOverridableProperty: " + normName + " = '" + value + "'");
        }
    ...
    private synchronized String getOverridableProperty(String name, String defval) {
        ...
        if (this.debug) System.err.println("UserPrefs.getOverridableProperty: Normalized name '" + normName + "'");
        ...
        if (this.debug) System.err.println("UserPrefs.getOverridableProperty: Looking in '" + this.name + "'...");

        value = this.locateProperty(this, normName);

        for (Properties defs = this.defaults; value == null && defs != null; ) {
            ...
            if (defs instanceof UserPrefs) {
                ...
                if (value == null) value = defs.getProperty(dNorm, null);
     ...
     *
     * @param name        The name of the property to retrieve.
     * @param defaultFont A default value.
    ...
    public Font getFont(String name, Font defaultFont) {
        ...
        String val = this.getProperty(name, null);
        if (this.debug) System.err.println("UserPrefs.getFont: property = " + val);

        if (val != null) {
            String[] flds = splitString(val, "-");
            if (this.debug) System.err.println("UserPrefs.getFont: flds.length = " + flds.length);

            if (flds.length == 1) {
                if (this.debug) System.err.println("UserPrefs.getFont: [0] " + flds[0]);
                ...
                try {
                    ...
                    if (flds[1].equalsIgnoreCase("BOLD")) style = Font.BOLD;
                    else if (flds[1].equalsIgnoreCase("ITALIC")) style = Font.ITALIC;
                    else if (flds[1].equalsIgnoreCase("BOLDITALIC")) style = Font.BOLD | Font.ITALIC;
                ...
                if (this.debug) System.err.println("UserPrefs.getFont: [0] " + flds[0] + " [1] " + flds[1]);
                try {
     ...
     *
     * @param name         The name of the property to retrieve.
     * @param defaultColor A default value.
    ...
    public Point getPoint(String name, Point defval) {
        ...
        String val = this.getProperty(name, null);
        if (val != null) {
            String[] flds = splitString(val, this.delim);
            if (flds.length == 2) {
                try {
                    Point p = new Point();
    ...
    public String[] getStringArray(String name, String[] defval) {
        List<String> v = this.getStringList(name, null);
        return v.toArray(new String[0]);
    }

    /**
     * Retrieve a system string List property list. String
     * Lists are represented by colon separated strings.
     * Returns a provided default value if the property
     * is not defined.
     *
     * @param name   The name of the property to retrieve.
    ...
    /**
     * NOTE That we use the ListString() method, since it will
    ...
    static public String[] splitString(String splitStr, String delim) {
        List<String> sv = StringUtilities.ListString(splitStr, delim);
        ...
        if (size > 0) {
            ...
            for (int row = 0; row < size; ++row) {
                ...
                if (key != null && rowSz > 0) {
                    ...
                    result.putTuple(new PrefsTuple(key, new ArrayList<>()));
                } else {
    ...
    public boolean isModified(String propName) {
        ...
        if (this.defaults != null) def = this.defaults.getProperty(propName);

        Object obj = this.getProperty(propName);

        if (obj == null) return false;

        if (def != null) return !obj.equals(def);
     ...
     *
     * @param propStr The property's value.
    ...
    public void setFont(String name, Font value) {
        ...
        String styleStr = (style == Font.PLAIN ? null : (style == Font.BOLD ? "BOLD" : (style == Font.ITALIC ? "ITALIC" : "BOLDITALIC")));
     ...
     *
     * @param name    The name of the property to set.
     * @param strings The property's value.
     */
     ...
     *
     * @param name  The name of the property to set.
     * @param tokes The String array of tokens.
     */
    ...
    public void setTokens(String name, String[] tokes) {
        ...
        for (int i = 0; i < tokes.length; ++i) {
            buf.append(tokes[i]);
            if (i < (tokes.length - 1)) buf.append(this.delim);
        }
     ...
     *
     * @param name  The name of the property to set.
     * @param table The property's value.
     */
    ...
    public void setTupleTable(String name, PrefsTupleTable table) {
        ...
        for (int i = 0; i < kv.size(); ++i) {
            PrefsTuple tup = table.getTuple((String) kv.get(i));
    ...
    public void removeProperty(String propName) {
        ...
        if (normName.endsWith(".")) normName = normName.substring(0, normName.length() - 1);
     ...
     *
     * @param out    The name of the property to set.
     * @param header The property's value.
     */
    ...
    public JPopupMenu loadPopupMenu(String menuPropertyName, ActionListener listener) {
    ...
    public void addMenuItems(JComponent menu, String menuPropertyName, ActionListener listener) {
        String itemString;
        String menuString;
        String itemNameStr;
        JMenuItem mItem;

        String[] itemList = this.getTokens("menu." + menuPropertyName, null);

        if (itemList == null || itemList.length == 0) {
            (new Throwable("Menu definition property '" + menuPropertyName + "' is not defined.")).printStackTrace();
            return;
        }
        ...
        for (String s : itemList) {
            ...
            if (itemString == null) {
                (new Throwable("Menu definition '" + menuPropertyName + "' is missing item definition property '" + itemNameStr + "'.")).printStackTrace();
            } else {
                ...
                if (itemString.equals("-")) {
                    ...
                    (new Throwable("Menu '" + menuPropertyName + "' Item '" + itemNameStr + "' has invalid definition.")).printStackTrace();
                } else {
