...
public class PrefsTupleTable extends Hashtable<String, PrefsTuple> {
    ...
    public boolean equals(PrefsTupleTable that) {
        ...
        for (Object o : this.keyOrder) {
            String key = (String) o;

            PrefsTuple thisTup = this.getTuple(key);
            PrefsTuple thatTup = that.getTuple(key);

            if (thisTup == null || thatTup == null) return false;

            if (!thisTup.equals(thatTup)) return false;
        }
    ...
    public PrefsTuple getTupleAt(int idx) {
        if (idx < 0 || idx >= this.keyOrder.size()) return null;

        return this.get(this.keyOrder.get(idx));
    }

    public PrefsTuple setTupleAt(PrefsTuple tup, int idx) {
        String key = (String) this.keyOrder.get(idx);

        PrefsTuple remTup = this.remove(key);
    ...
    public void removeTupleAt(int idx) {
        if (idx < 0 || idx >= this.keyOrder.size()) return;
    ...
    public void putTuple(PrefsTuple tuple) {
        if (this.get(tuple.getKey()) == null) this.keyOrder.add(tuple.getKey());
    ...
    public int getMaximumTupleLength() {
        ...
        for (PrefsTuple tup : this.values()) {
            if (tup.length() > max) max = tup.length();
        }
