...
public class CVSLog {
    ...
    static {
        ...
        CVSLog.filename = CVSLog.DEFAULT_FILENAME;
    }
    ...
    static public void setAutoFlush(boolean autoflush) {
        CVSLog.autoFlush = autoFlush;
    }

    static public void checkLogOpen() {
        if (CVSLog.checked) return;
    ...
    static public void openLogFile() {
        ...
        if (CVSLog.debugOpen) (new Throwable("OPEN CVS LOG")).printStackTrace();

        if (CVSLog.debug) System.err.println("CVSLog.openLogFile( " + CVSLog.filename + " )");

        if (CVSLog.filename == null) return;

        try {
            CVSLog.file = new FileWriter(CVSLog.filename);
        } catch (Exception ex) {
            CVSLog.logMsg("error opening log file '" + CVSLog.filename + "', trying 'user.dir' - " + ex.getMessage());

            String userDirStr = System.getProperty("user.dir", "");

            CVSLog.filename = userDirStr + File.separator + CVSLog.DEFAULT_FILENAME;

            try {
                CVSLog.file = new FileWriter(CVSLog.filename);
            } catch (Exception ex2) {
                CVSLog.logMsg("error opening log file '" + CVSLog.filename + "' - " + ex2.getMessage());
    ...
    static public void setEcho(boolean setting) {
        CVSLog.echo = setting;
    }

    static public void traceMsg(Throwable thrown, String msg) {
        CVSLog.logMsg(msg);
        CVSLog.logMsg(thrown.getMessage());

        if (!CVSLog.open) thrown.printStackTrace(System.err);
        else thrown.printStackTrace(CVSLog.stream);

        if (CVSLog.autoFlush && CVSLog.open) CVSLog.stream.flush();
    }
    ...
    static public void logMsgStdout(String msg) {
        CVSLog.checkLogOpen();
    ...
    static public void logMsgStderr(String msg) {
        CVSLog.checkLogOpen();
